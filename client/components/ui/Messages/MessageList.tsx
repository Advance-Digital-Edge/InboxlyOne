      ...
      <div className="p-4">
        ...
        <div className="space-y-3">
          {messages.length > 0 ? messages.map((message) => {
            ...
            const temporaryMessagesArray = temporaryMessages ? Array.from(temporaryMessages.values()) : [];
            const hasSendingMessage = temporaryMessagesArray.some((tempMsg) => {
              return tempMsg.isIncoming === false && (
                (message as any).channelId === (selectedMessage as any)?.channelId ||
                message.senderId === selectedMessage?.senderId
              );
            });
            
            // Get the sending status for visual feedback
            const sendingTempMessage = temporaryMessagesArray.find(tempMsg => 
            ...
            const sendingStatus = sendingTempMessage ? messageStatuses?.get(sendingTempMessage.id) : undefined;
